...
static unsigned long clk_pll_recalc_rate(struct clk_hw *hw,unsigned long parent_rate)
{
    ...
    if(rate_int == pll_cfg->target_rate_int) {
   	    return pll_cfg->target_rate;
    } else if(rate_int > pll_cfg->target_rate_int) {
   	    return pll_cfg->max_rate;
    }
...
void axera_clk_register_pll(struct axera_pll_cfg_reg *clks, int nums, struct axera_clock_data *data)
{
	...
	p_clk->target_rate_int = clks[i].target_rate_int;
        p_clk->target_rate = clks[i].target_rate;
