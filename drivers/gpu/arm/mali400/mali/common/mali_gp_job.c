...
static struct mali_gp_job *__mali_gp_job_create(struct mali_session_data *session, _mali_uk_gp_start_job_s *uargs, u32 id, struct mali_timeline_tracker *pp_tracker, u32 usize)
{
	...
	if (NULL != job) {
		...
		memset(&job->uargs, 0, sizeof(_mali_uk_gp_start_job_s));
		if (0 != _mali_osk_copy_from_user(&job->uargs, uargs, usize)) {
			goto fail1;
		}
...
{
	return __mali_gp_job_create(session, uargs, id, pp_tracker, sizeof(_mali_uk_gp_start_job_s));
}

struct mali_gp_job *mali_gp_job_create_v800(struct mali_session_data *session, _mali_uk_gp_start_job_v800_s *uargs, u32 id, struct mali_timeline_tracker *pp_tracker)
{
	return __mali_gp_job_create(session, (_mali_uk_gp_start_job_s *)uargs, id, pp_tracker, sizeof(_mali_uk_gp_start_job_v800_s));
}
